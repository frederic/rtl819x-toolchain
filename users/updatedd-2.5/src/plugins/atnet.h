...
#define DYNDNSHOST	"atddns.hs.home.ne.jp"
#define ATNETDDNS_RESULT_FILE "/var/tmp/ddns_result.html"
#define PORT		80
...
static int update_atnet(const int s, struct arguments *args);
static int clear_atnet(const int s, struct arguments *args);
static int check_server_msg(const int s, const char *hostnames);

char *
atnet_get_retmsg(void)
{
	return ret_msg_buf;
}
