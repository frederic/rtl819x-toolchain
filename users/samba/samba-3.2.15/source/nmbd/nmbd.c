...
static bool reload_nmbd_services(void)
{
	bool ret;
 ...
 int main(int argc, char * const argv[])
{
	...
	while ((opt = getopt (argc, argv, "d:s:Dih")) != -1) {
		switch (opt) {
		case 'd':
#ifndef AVM_NO_DEBUG
			if (optarg) {
	...
	if (!override_logfile) {
		char *logfile = NULL;
		if (asprintf(&logfile, "%s/log.nmbd", LOGFILEBASE) < 0) {
			exit(1);
		}
	...
	if (!lp_load_initial_only(dyn_CONFIGFILE)) {
