...
{
#if !defined AVM_NO_DEBUG && defined DEBUG_PASSWORD
	DEBUG(11, ("%s", msg));
	if (data != NULL && len > 0)
	{
...
{
	return talloc_asprintf(talloc_tos(), "%s/%s", LIBDIR, name);
}
...
{
	return talloc_asprintf(talloc_tos(), "%s/%s", CODEPAGEDIR, name);
}
