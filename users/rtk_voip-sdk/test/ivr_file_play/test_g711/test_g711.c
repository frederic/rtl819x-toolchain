...
int rtk_SetIvrPlayG711( unsigned int nCount, const unsigned char *pData )
{
...
lable_put_g711_data:
		cWritten = rtk_SetIvrPlayG711( cRead, buffer + shift );
		//printf( "Write:%d, %d\n", cRead, shift );
		...
		if( cWritten < cRead ) {
			...
			rtk_SetIvrPlayG711( 0, buffer );	/* show current playing time */
