...
static int create_nameip(nameip_t *nameip, char *ip)
{
    ...
    ipnum.s_addr = (((nameip->ipnum & 0xff000000) >> 24) |((nameip->ipnum & 0x00ff0000) >> 8) | ((nameip->ipnum & 0x0000ff00) << 8) | ((nameip->ipnum & 0x000000ff) << 24));
/*    
...
char *get_hostname(char **from, char *domain, char *name, int size)
{
    ...
    *name = 0;
    ...
    if (*get_word_hosts(from, word, sizeof(word)) == 0)
#else
...
int read_hosts(char *filename, char *domain)
{
	...
	if (isdigit((int)(*p))) {
	     ...
	     */
#if defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_DOMAIN_NAME_QUERY_SUPPORT) || defined(CONFIG_RTL8196C_EC)
	    get_word_hosts(&p, ipnum, sizeof(ipnum));
#else
...
int Check_DomainName(char *Query_Name, char *Domain_Name)
{
	char *token=NULL, *savestr1=NULL;
	char tmpBuff[200];
	int result=0;
	char *domain_token=NULL, *domain_name=NULL;
	char tmpBuff_domainname[200];
	
	if(strcmp(Domain_Name, "alwayshost") == 0)
	{
		return 1;
	}
	...
	domain_token = NULL;
	domain_name=NULL;
	sprintf(tmpBuff_domainname, "%s", Domain_Name); 	
	...
	while (domain_token != NULL)
	{
		...
		token=NULL;
		savestr1=NULL;		 
		sprintf(tmpBuff, "%s", Query_Name);
		if (strcmp(tmpBuff , domain_token)==NULL)
			return 1;
...
static dnsrec_t *name_lookup(char *name)
{
    ...
    dnsrec_t *rec;
#if defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL865X_SC) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_DOMAIN_NAME_QUERY_SUPPORT) || defined(CONFIG_RTL8196C_EC)
    char *str_tok, *check_com=NULL;
    char hostname_lower[100];
    unsigned int hostname_code;
...
#if defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL865X_SC) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_DOMAIN_NAME_QUERY_SUPPORT) || defined(CONFIG_RTL8196C_EC)
    if(HOSTNAME[0]){
    	sprintf(hostname_lower, "%s", HOSTNAME);
    	 strlwr(hostname_lower);
		if(Check_DomainName(name, hostname_lower)==1){
		 		hostname_code = get_stringcode(HOSTNAME);
			    	for (i = 0; i < dbc; i++) {
					rec = dbv[i];
					if ((rec->type == DNS_NAMEIP)  && (rec->object.code == hostname_code) &&(strcmp(rec->object.string, hostname_lower) == 0)) {
				    		return (rec);
					}
				}	
			}
...
#if defined(CONFIG_RTL865X_SC)	 || defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD)  || defined(CONFIG_DOMAIN_NAME_QUERY_SUPPORT) || defined(CONFIG_RTL8196C_EC)
	else{
		//Brad: we read /etc/hosts file even no resolv.conf in sc case 
		 read_hosts("/etc/hosts", domain);
	}
#endif	
    }
...
extern char domain_tbl[20][40];
extern int domain_filter;
extern int total_entryDomain;

int find_domain(char *data, int dlen, char *pattern, int plen, char term)
{	int i;	
 ...
 ***********************************************************
 * Brad add a argument int *Action to check whether block the query or not.
 ************************************************************
 */
#if defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL865X_SC) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_DOMAIN_NAME_QUERY_SUPPORT) || defined(CONFIG_RTL8196C_EC)
int master_lookup(unsigned char *msg, int len, int *Action, char *comment)
#else
...
//Brad add for domain blocking
	if(domain_filter !=0){
		for(i=0; i< total_entryDomain ;i++) {	   
			 found = find_domain(query.name,strlen(query.name), domain_tbl[i], strlen(domain_tbl[i]),'\0');	   
			if(found){
				//printf("Got the match case :%s <----->%s\n",query.name ,domain_tbl[i]);
				if(domain_filter==1){
					sprintf(comment, "%s", domain_tbl[i]);
				}
			      break;
			}
		}
	...
	if(domain_filter==1){
		if(found==1){
			...
			*Action=0;
	}else if(domain_filter==2){
			if(found==1)
				*Action=0;
			else{
				*Action=1;
				sprintf(comment, "%s", query.name);
				return 0;
			}
		}else 
			*Action=0;
