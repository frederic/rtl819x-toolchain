...
int read_interface_netmask(char *interface,u_int32_t *netmask)
{
	int fd;
	struct ifreq ifr;
	...
	if((fd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) >= 0) 
	{
		...
		{ 
			...
			{
				our_mask = (struct sockaddr_in *) &ifr.ifr_netmask;
				*netmask = our_mask->sin_addr.s_addr;
				//printf( "+++++++++%s (our netmask) = %s+++++++++\n", ifr.ifr_name, inet_ntoa(our_mask->sin_addr));
			} else {
				LOG(LOG_ERR, "SIOCGIFADDR failed, is the interface up and configured?: %s", 
						strerror(errno));
				close(fd);
