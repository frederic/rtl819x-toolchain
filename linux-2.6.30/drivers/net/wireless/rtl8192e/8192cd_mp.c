...
{
	int baseA = (pwrA << 24) | (pwrA << 16) | (pwrA << 8) | (pwrA);
	int baseB = (pwrB << 24) | (pwrB << 16) | (pwrB << 8) | (pwrB);

	RTL_W32(rTxAGC_A_Ofdm18_Ofdm6_JAguar, baseA);
	RTL_W32(rTxAGC_A_Ofdm54_Ofdm24_JAguar, baseA);
	RTL_W32(rTxAGC_A_MCS3_MCS0_JAguar, baseA);
	RTL_W32(rTxAGC_A_MCS7_MCS4_JAguar, baseA);
	RTL_W32(rTxAGC_A_MCS11_MCS8_JAguar, baseA);
	RTL_W32(rTxAGC_A_MCS15_MCS12_JAguar, baseA);
	RTL_W32(rTxAGC_A_Nss1Index3_Nss1Index0_JAguar, baseA);
	RTL_W32(rTxAGC_A_Nss1Index7_Nss1Index4_JAguar, baseA);
	RTL_W32(rTxAGC_A_Nss2Index1_Nss1Index8_JAguar, baseA);
	RTL_W32(rTxAGC_A_Nss2Index5_Nss2Index2_JAguar, baseA);
	RTL_W32(rTxAGC_A_Nss2Index9_Nss2Index6_JAguar, baseA);

	RTL_W32(rTxAGC_B_Ofdm18_Ofdm6_JAguar, baseB);
	RTL_W32(rTxAGC_B_Ofdm54_Ofdm24_JAguar, baseB);
	RTL_W32(rTxAGC_B_MCS3_MCS0_JAguar, baseB);
	RTL_W32(rTxAGC_B_MCS7_MCS4_JAguar, baseB);
	RTL_W32(rTxAGC_B_MCS11_MCS8_JAguar, baseB);
	RTL_W32(rTxAGC_B_MCS15_MCS12_JAguar, baseB);
	RTL_W32(rTxAGC_B_Nss1Index3_Nss1Index0_JAguar, baseB);
	RTL_W32(rTxAGC_B_Nss1Index7_Nss1Index4_JAguar, baseB);
	RTL_W32(rTxAGC_B_Nss2Index1_Nss1Index8_JAguar, baseB);
	RTL_W32(rTxAGC_B_Nss2Index5_Nss2Index2_JAguar, baseB);
	RTL_W32(rTxAGC_B_Nss2Index9_Nss2Index6_JAguar, baseB);

	RTL_W32(rTxAGC_A_CCK11_CCK1_JAguar, baseA);
	RTL_W32(rTxAGC_B_CCK11_CCK1_JAguar, baseB);
	...
	while (1)
	{
		...
		if ((OPMODE & WIFI_MP_CTX_SCR) &&
			...
			if (CHECKICIS8812() || CHECKICIS8881A()) {
				...
				PHY_SetBBReg(priv, 0x914, BIT(17), bEnable);		// single carrier tx
				PHY_SetBBReg(priv, 0x914, BIT(18), bEnable);		// single tone tx
				
				PHY_SetBBReg(priv, 0x914, 0xffff, 0x404);
			} else 		
#endif
			{
				// 1. if OFDM block on?
...
static void mp_chk_sw_ant_AC(struct rtl8192cd_priv *priv)
{
	...
	*/
	...
	case ANTENNA_B:
		...
		PHY_SetBBReg(priv, 0xa07, 0xc, 0x2);	
...
void mp_set_phyBand(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if((GET_CHIP_VER(priv)==VERSION_8812E) || (GET_CHIP_VER(priv)==VERSION_8881A))
	{
		...
		/* 
	 	 ...
	 	 */
		PHY_SetBBReg(priv, 0x808, 0xff, 0x33);
		PHY_SetBBReg(priv, 0xa07, BIT(2) | BIT(3), 0x3);
