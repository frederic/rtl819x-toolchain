...
void choose_IOT_main_sta(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if ((GET_ROOT(priv)->up_time % 2) == 0) {
		...
		((IS_OUTSRC_CHIP(priv)&&(pstat->IOTPeer==HT_IOT_PEER_INTEL)) 
...
#ifdef _OUTSRC_COEXIST
			if((IS_OUTSRC_CHIP(priv)&&(pstat && pstat->IOTPeer==HT_IOT_PEER_INTEL && (((pstat->tx_avarage+pstat->rx_avarage)>>10) < 
					priv->pshare->rf_ft_var.cwmax_enhance_thd))) 
				|| (!IS_OUTSRC_CHIP(priv) && (pstat && pstat->is_intel_sta && (((pstat->tx_avarage+pstat->rx_avarage)>>10) < 
					priv->pshare->rf_ft_var.cwmax_enhance_thd))))
#else
#ifdef USE_OUT_SRC
