...
// This should be moved out of OUTSRC
	...
	{
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(), write 0x%x for BT HS mode!!!\n", 
			pDM_Odm->btHsDigVal));
		ODM_Write_DIG(pDM_Odm, pDM_Odm->btHsDigVal);
		return	TRUE;
	}
#endif	
...
#if ((RTL8192C_SUPPORT==1) || (RTL8192D_SUPPORT==1) || (RTL8188E_SUPPORT==1) || (RTL8192E_SUPPORT==1))
			...
			{			
...
#endif
				{
					...
					if(pDM_Odm->SupportICType == ODM_RTL8812) {
						...
						FillH2CCmd8812(pDM_Odm->priv, H2C_8812_RSSI_REPORT, 3, H2C_Parameter);
					}
#endif
					...
					if(pDM_Odm->SupportICType == ODM_RTL8881A || pDM_Odm->SupportICType == ODM_RTL8192E) {
						...
						H2C_Parameter[2] = (u1Byte)(pstat->rssi & 0xFF);
						H2C_Parameter[0] = REMAP_AID(pstat);
                        GET_HAL_INTERFACE(pDM_Odm->priv)->FillH2CCmdHandler(pDM_Odm->priv, H2C_88XX_RSSI_REPORT, 3, H2C_Parameter);
					}
#endif
				
#if CONFIG_RTL_92C_SUPPORT || defined(CONFIG_RTL_92D_SUPPORT)	
	...
	)
{
	...
	if ((GET_ROOT(priv)->up_time % 2) == 0) {
		...
		if((IS_OUTSRC_CHIP(priv)&&((pstat->IOTPeer==HT_IOT_PEER_RALINK) && ((tx_2s_avg + rx_2s_avg) >= 45))) 
			|| (!IS_OUTSRC_CHIP(priv) && (pstat->is_ralink_sta && ((tx_2s_avg + rx_2s_avg) >= 45))))
#else
			...
			if (pstat && pstat->rssi >= priv->pshare->rf_ft_var.txop_enlarge_upper) {
#ifdef LOW_TP_TXOP
#ifdef _OUTSRC_COEXIST
				if((IS_OUTSRC_CHIP(priv)&&(pstat->IOTPeer==HT_IOT_PEER_INTEL)) 
					|| (!IS_OUTSRC_CHIP(priv) && (pstat->is_intel_sta)))
#else
#if (DM_ODM_SUPPORT_TYPE &ODM_AP) && defined(USE_OUT_SRC)
				...
				if (priv->pshare->txop_enlarge != 2)
				{
					...
					if (pstat->is_intel_sta)
						...
						priv->pshare->txop_enlarge = 0xe;
#ifdef _OUTSRC_COEXIST
					else if((IS_OUTSRC_CHIP(priv)&&(pstat->IOTPeer==HT_IOT_PEER_RALINK)) 
						|| (!IS_OUTSRC_CHIP(priv) && (pstat->is_ralink_sta)))
#else
#if (DM_ODM_SUPPORT_TYPE &ODM_AP) && defined(USE_OUT_SRC)
...
//#if (defined CONFIG_RTL_819XD))
			...
			if (pstat && (pstat->ADDBA_ready[0] | pstat->ADDBA_ready[3]) &&(pstat->IOTPeer == HT_IOT_PEER_INTEL)) { 	
				//int en_thd = 14417920>>(priv->up_time % 2);
				avg_tp = ((pstat->current_tx_bytes >> 17) >> (priv->up_time % 2 == 0));
				
				//if ((priv->swq_en == 0) && (pstat->current_tx_bytes > en_thd) && (pstat->current_rx_bytes > en_thd) )  { //50Mbps
				if ((priv->swq_en == 0) && (avg_tp  > (TP_HIGH_WATER_MARK+5))) { //55Mbps
					//printk("[%s:%d] swq_en=1 current_tp: %d Mbps\n", __FUNCTION__, __LINE__, avg_tp);
					priv->swq_en = 1;
					priv->swqen_keeptime = priv->up_time;
				}
				//else if ((priv->swq_en == 1) && ((pstat->tx_avarage < 4587520) || (pstat->rx_avarage < 4587520))) { //35Mbps
				else if ((priv->swq_en == 1) && (avg_tp < (TP_LOW_WATER_MARK+5))) { //35Mbps				
					//printk("[%s:%d] swq_en=0 current_tp: %d Mbps\n", __FUNCTION__, __LINE__, avg_tp);
					priv->swq_en = 0;
