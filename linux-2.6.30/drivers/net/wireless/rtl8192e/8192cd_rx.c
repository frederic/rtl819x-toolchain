...
static __inline__ void translate_rssi_sq_outsrc(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, char rate)
{
    ...
    if ((GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8881A)) {
        unsigned short *t = get_pframe(pfrinfo)+2;
        *t = le16_to_cpu(*t);
    }
#endif
...
static void check_igmp_snooping_pkt( struct sk_buff *pskb )
{
	...
	if (IS_MCAST(dest)) 
	{
		...
		{
			iph = (struct iphdr *)(skb_mac_header(pskb) + ETH_HLEN);
...
{
	struct rtl8192cd_priv *priv;
#if defined(__LINUX_2_6__) || defined(SMP_SYNC) || defined(__ECOS)
...
#ifdef CONFIG_WLAN_HAL
//                cur_q->host_idx != cur_q->cur_host_idx) {
                IS_HAL_CHIP(priv) ? 
...
#ifdef  CONFIG_WLAN_HAL
        if (IS_HAL_CHIP(priv)) {
            //Filen: It is not necessary to clear HISR
        } else
#endif  //CONFIG_WLAN_HAL
        {
#ifdef CONFIG_RTL_88E_SUPPORT
...
void rtl88XX_rx_isr(struct rtl8192cd_priv *priv)
{
 ...
 // Pedro, the following code copy from rtl8192cd_rx_isr, but these two compile flags don't use currently.
        ...
        if (rx_desc_status.CRC32) {
            ...
            if (!(rx_desc_status.FS == 0x01 && rx_desc_status.LS == 0x01)) {
                printk("%s(%d): (%d,%d) \n", __FUNCTION__, __LINE__, rx_desc_status.FS, rx_desc_status.LS);
            ...
            if ( ((!(priv->pmib->dot11BssType.net_work_type & WIRELESS_11AC)) && (pfrinfo->pktlen > 0x2000))
                    || ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11AC) && (pfrinfo->pktlen > 0x2d00))
                    || (pfrinfo->pktlen < 16)) {
#if defined(RX_BUFFER_GATHER)
				if (!(pfrinfo->gather_flag && (pfrinfo->pktlen < 16)))
#endif
				{
				//	printk("pktlen is not in range, pfrinfo->pktlen=%d(0x%x), goto rx_reuse\n",pfrinfo->pktlen, pfrinfo->pktlen);
					goto rx_reuse;
				}
			}
...
#if 0   //Filen_Debug
            if ((unsigned int)pskb->data & BIT0) {
                printk("Error: pskb->data(0x%x), pskb->head(0x%x), pskb->tail(0x%x), pskb(0x%x)\n", pskb->data, pskb->head, pskb->tail, pskb);
                printk("Error: rxbuf_shift(0x%x), driver_info_size(0x%x)\n", pfrinfo->rxbuf_shift, pfrinfo->driver_info_size);
            }
#endif

            // TODO: Pedro: the following setting in rx buffer gather only to be set when GATHER_STATE_FIRST ?
	...
	//-------------------------------------------------------------------------------------------
                ...
                pskb->data -= skb_shift_size; // skb_shift_size == (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + sizeof(RX_DESC_88XX))
                pskb->tail -= skb_shift_size; // skb_shift_size == (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + sizeof(RX_DESC_88XX))
			}
			else
...
#if defined(RX_BUFFER_GATHER)
                if (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + pfrinfo->pktlen <=
                    (RX_BUF_LEN- sizeof(struct rx_frinfo)-RTL_WLAN_DRV_RX_GATHER_GAP_THRESHOLD)) {
#else
				if (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + pfrinfo->pktlen + _CRCLNG_ <=
                    (RX_BUF_LEN- sizeof(struct rx_frinfo)-RTL_WLAN_DRV_RX_GATHER_GAP_THRESHOLD)) {
#endif // RX_BUFFER_GATHER
					skb_reserve(pskb, (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size));
                    skb_shift_size += (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size);
...
//                            printk("%s(%d): icv error rx_reuse\n", __FUNCTION__, __LINE__);
					...
					if (priv->pshare->gather_state != GATHER_STATE_NO) {
						...
						if (priv->pshare->gather_state == GATHER_STATE_LAST) {
                            ...
                            printk("%s(%d): reuse:0x%x, after search_first_segment\n", __FUNCTION__, __LINE__, reuse);
							priv->pshare->gather_state = GATHER_STATE_NO;
						} else {
							reuse = 0;
						}
...
#endif
                    {
//                         printk("%s(%d): reuse:0x%x, before validate_mpdu\n", __FUNCTION__, __LINE__, reuse);
                         reuse = validate_mpdu(priv, pfrinfo);
//                         printk("%s(%d): reuse:0x%x, after validate_mpdu\n", __FUNCTION__, __LINE__, reuse);
                    }
...
#if defined(DELAY_REFILL_RX_BUF)
            GET_HAL_INTERFACE(priv)->UpdateRXBDInfoHandler(priv, q_num,
                (cur_q->host_idx+cur_q->rxbd_ok_cnt)%cur_q->total_rxbd_num, pskb, init_rxdesc_88XX, _FALSE);
#else
...
rx_reuse:
    #if 0   //Filen: for debug
    {
		...
		if (cmp_flags)
		{
			...
			skb_reserve(pskb, 4);
#endif
//            printk("%s(%d): call refill_rx_ring_88XX, rxbd_ok_cnt:%d \n", __FUNCTION__, __LINE__, cur_q->rxbd_ok_cnt);
...
#if defined(CONFIG_NET_PCI) && !defined(USE_RTL8186_SDK)
            rtl_cache_sync_wback(priv, bus_to_virt(phw->rx_infoL[cur_q->cur_host_idx].paddr),
...
rx_reuse:
...
#endif	/*	defined(DELAY_REFILL_RX_BUF)	*/
		{
			...
			rtl_cache_sync_wback(priv, (unsigned int)bus_to_virt(phw->rx_infoL[tail].paddr), RX_BUF_LEN - sizeof(struct rx_frinfo)-64, PCI_DMA_FROMDEVICE);
...
void rtl8192cd_rx_dsr(unsigned long task_priv)
{
	struct rtl8192cd_priv	*priv = (struct rtl8192cd_priv *)task_priv;
	unsigned long flags;
	unsigned long mask, mask_rx, mask_ext, mask_ext_rx;
#ifdef MBSSID
	int i;
#endif
