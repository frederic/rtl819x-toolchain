...
#ifdef _OUTSRC_COEXIST
	if((IS_OUTSRC_CHIP(priv)&&(priv->pshare->is_40m_bw && (pstat->IOTPeer == HT_IOT_PEER_MARVELL))) 
		|| (!IS_OUTSRC_CHIP(priv) && (priv->pshare->is_40m_bw && pstat->is_marvell_sta == 1)))
#else
#ifdef USE_OUT_SRC	
...
#if defined(TXREPORT) && (defined(CONFIG_RTL_92C_SUPPORT) || defined(CONFIG_RTL_92D_SUPPORT)) || defined(USE_OUT_SRC)
...
// Client mode IOT issue, Button 2009.07.17
	...
	{
#ifdef _OUTSRC_COEXIST
		if((IS_OUTSRC_CHIP(priv)&&(!pstat->is_realtek_sta && (pstat->IOTPeer != HT_IOT_PEER_MARVELL) && pstat->is_legacy_encrpt)) 
			|| (!IS_OUTSRC_CHIP(priv) && (!pstat->is_realtek_sta && !pstat->is_marvell_sta && pstat->is_legacy_encrpt)))
#else
#ifdef USE_OUT_SRC	
...
#ifdef RTK_AC_SUPPORT  //80M , todo, limlt in Band 1 
#if 1 //Enable all channels 
static struct channel_list reg_channel_5g_band_AC[] = {
	/* FCC */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* IC */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* ETSI */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* SPAIN */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* FRANCE */{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* MKK */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* ISRAEL */	{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* MKK1 */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* MKK2 */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* MKK3 */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* NCC (Taiwan) */	{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* RUSSIAN */	{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
	/* CN */		{{36,40,44,48, 52,56,60,64, 100,104,108,112, 116,120,124,128, 132,136,140,144, 149,153,157,161, 165,169,173,177}, 28},
};
#else
static struct channel_list reg_channel_5g_band_AC[] = {
	...
	/* MKK1 */		{{36,40,44,48,52,56,60,64},8},
	/* MKK2 */		{{36,40,44,48,52,56,60,64},8},
	/* MKK3 */		{{36,40,44,48,52,56,60,64},8},
	/* NCC (Taiwan) */	{{36,40,44,48,52,56,60,64},8},
...
//------------------------------------------------------ 2007-04-14
	...
	if((GET_CHIP_VER(priv) == VERSION_8812E)||(GET_CHIP_VER(priv) == VERSION_8881A)) {
		if(priv->pmib->dot11RFEntry.phyBandSelect != PHY_BAND_2G) {
			for (i=0; i<reg_channel_5g_band_AC[reg-1].len; i++)
				priv->available_chnl[i] = reg_channel_5g_band_AC[reg-1].channel[i];
		
			priv->available_chnl_num = reg_channel_5g_band_AC[reg-1].len;
		} else	{
			for (i=0; i<reg_channel_2_4g[reg-1].len; i++)
				priv->available_chnl[i] = reg_channel_2_4g[reg-1].channel[i];
...
#ifdef _OUTSRC_COEXIST
		if((IS_OUTSRC_CHIP(priv)&&(priv->pshare->is_40m_bw && (pstat->IOTPeer == HT_IOT_PEER_MARVELL))) 
			|| (!IS_OUTSRC_CHIP(priv) && ( priv->pshare->is_40m_bw && pstat->is_marvell_sta)))
#else
#ifdef USE_OUT_SRC	
			if (priv->pshare->is_40m_bw && (pstat->IOTPeer == HT_IOT_PEER_MARVELL))
#else
...
void txrpt_handler(struct rtl8192cd_priv *priv, struct tx_rpt *report)
{
	...
	if( ((GET_CHIP_VER(priv)== VERSION_8812E) || GET_CHIP_VER(priv) == VERSION_8192E) 
		&& pstat->aid && (pstat->aid < FW_NUM_STAT-1))
	{
